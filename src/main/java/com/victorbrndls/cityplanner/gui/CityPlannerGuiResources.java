package com.victorbrndls.cityplanner.gui;

import net.minecraft.resources.ResourceLocation;

public class CityPlannerGuiResources {

    public static final ResourceLocation PLANK_RESOURCE =
            new ResourceLocation("cityplanner", "textures/gui/resource_plank.png");
    public static final ResourceLocation WATER_RESOURCE =
            new ResourceLocation("cityplanner", "textures/gui/resource_water.png");
    public static final ResourceLocation ENERGY_RESOURCE =
            new ResourceLocation("cityplanner", "textures/gui/resource_energy.png");
    public static final ResourceLocation VEGETABLE_RESOURCE =
            new ResourceLocation("cityplanner", "textures/gui/resource_vegetable.png");
    public static final ResourceLocation FURNITURE_RESOURCE =
            new ResourceLocation("cityplanner", "textures/gui/resource_furniture.png");

    public static final ResourceLocation RESIDENT_RESOURCE =
            new ResourceLocation("cityplanner", "textures/gui/resource_resident.png");
    public static final ResourceLocation RESIDENT_SATISFACTION =
            new ResourceLocation("cityplanner", "textures/gui/resident_satisfaction.png");

}
